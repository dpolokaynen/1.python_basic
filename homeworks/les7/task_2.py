...
Реализовать проект расчета суммарного расхода ткани на производство одежды. Основная сущность
 (класс) этого проекта — одежда, которая может иметь определенное название. К типам одежды
  в этом проекте относятся пальто и костюм. У этих типов одежды существуют параметры:
...
class Clothes:
    ...
    def cost(self):
        if self.name == 'coat':
            return print(int(self.size) / 6.5 + 0.5)
        if self.name == 'costume':
            return print(int(self.height) * 2 + 0.3)
test_clothes_1 = Clothes('coat',52,20)
test_clothes_1.cost
test_clothes_2 = Clothes('costume',52,20)
test_clothes_2.cost
